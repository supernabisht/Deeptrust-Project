...
class DeepfakeEvaluator:
    ...
    def align_features(self, X, feature_names=None):
        """Align features to match model's expected format."""
        if feature_names is None:
            feature_names = self.feature_names
            
        if feature_names is None and hasattr(self.model, 'feature_names_in_'):
    ...
    def evaluate(self, X_test, y_test):
        ...
        try:
            ...
            if hasattr(self.model, 'n_features_in_') and X_test.shape[1] != self.model.n_features_in_:
                print(f"\nFeature mismatch: Model expects {self.model.n_features_in_} features, got {X_test.shape[1]}")
    ...
    def plot_pr_curve(self, recall, precision, pr_auc):
        ...
        plt.close()
        print("Precision-Recall curve saved to 'reports/pr_curve.png'")
